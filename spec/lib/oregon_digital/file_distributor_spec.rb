...
RSpec.describe OregonDigital::FileDistributor do
  let(:fd) { OregonDigital::FileDistributor.new("~45xzy4") }
  ...
  describe "#identifier" do
    it "should convert non-alphanumerics to hyphens" do
      expect(fd.identifier).to eq("-45xzy4")
    end
  end
  ...
  describe "#filename" do
    it "should combine scrubbed identifier and extension to produce a filename" do
      fd.extension = ".jar"
      expect(fd.filename).to eq("-45xzy4.jar")
    end
  end
