...
RSpec.describe Files::YmlFile do
  subject(:datastream) {Files::YmlFile.new}
  ...
  describe ".to_solr" do
    ...
    context "when there is content" do
      ...
      it "should return all the root keys that have content" do
        expect(asset.to_solr.keys).to include("descMetadata__third_root_teim")
      end
      it "should index child keys" do
        expect(asset.to_solr.keys).to include("descMetadata__oregondigital__test_key_teim")
      end
      it "should index arrays" do
        expect(asset.to_solr["descMetadata__oregondigital__test_key_teim"]).to eq ["5","6","7"]
      end
      it "should be included in the asset's solr information" do
        expect(asset.to_solr).to include("descMetadata__oregondigital__test_key_teim")
      end
    end
  end
  ...
  describe "asset.save" do
    context "when it has content" do
      ...
      it "should store the data in Fedora" do
        reloaded_asset = DummyAsset.find(asset.id)
